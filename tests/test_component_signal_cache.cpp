...
int main() {
...

    {
        ...
        std::vector<const bam1_t*> low_mapq_records = {
            low_mapq_indel_record.get(),
            low_mapq_left_clip_record.get(),
            low_mapq_right_clip_record.get(),
        };
        std::vector<ReadReferenceSpan> low_mapq_spans = {
            make_span(0, 50, 150),
            make_span(0, 95, 205),
            make_span(0, 0, 105),
        };

        ComponentCall low_mapq_component;
        low_mapq_component.chrom = "chr1";
        low_mapq_component.tid = 0;
        low_mapq_component.anchor_pos = 100;

        const auto low_mapq_cache = pipeline.build_component_signal_cache(
            low_mapq_records,
            low_mapq_spans,
            low_mapq_component.chrom);
        const EventReadEvidence low_mapq_old = pipeline.collect_event_read_evidence_for_bounds(
            low_mapq_component,
            low_mapq_records,
            low_mapq_spans,
            {},
            ...
            100,
            100);
        const EventReadEvidence low_mapq_cached =
            pipeline.collect_event_read_evidence_for_bounds_cached(
                low_mapq_component,
                low_mapq_cache,
                {},
                100,
        ...
        assert(low_mapq_old.alt_indel_reads == 0);
        assert(low_mapq_old.alt_left_clip_reads == 1);
        assert(low_mapq_old.alt_right_clip_reads == 1);
        assert(low_mapq_old.alt_struct_reads == 2);
        assert(low_mapq_old.support_qnames.size() == 2);
        assert(low_mapq_old.support_qnames[0] == "low_mapq_left_clip_support");
        assert(low_mapq_old.support_qnames[1] == "low_mapq_right_clip_support");
        assert(low_mapq_old.alt_indel_reads == low_mapq_cached.alt_indel_reads);
        assert(low_mapq_old.alt_struct_reads == low_mapq_cached.alt_struct_reads);
        assert(low_mapq_old.support_qnames == low_mapq_cached.support_qnames);
    }
