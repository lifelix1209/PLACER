...
int main() {
    ...
    {
        const auto low_mapq_read = make_record(
            "low_mapq_ins",
            0,
            1000,
            59,
        ...
        std::vector<const bam1_t*> records = {low_mapq_read.get()};
...

    {
        ...
        assert(components.front().breakpoint_candidates.front().ins_len == 60);
        assert(components.front().breakpoint_candidates.front().clip_len == 0);
        assert(
...

    {
        ...
        assert(components.front().breakpoint_candidates.size() == 1);
        assert(components.front().breakpoint_candidates.front().ins_len == 0);
        assert(components.front().breakpoint_candidates.front().clip_len == 60);
        assert(
            (components.front().breakpoint_candidates.front().class_mask &
             kCandidateSoftClip) != 0);
    }
