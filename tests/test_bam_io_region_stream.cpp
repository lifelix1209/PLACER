...
int main() {
    ...
    placer::BamRegionScope alias_scope;
    alias_scope.enabled = true;
    alias_scope.chrom = "1";
    alias_scope.start = 0;
    alias_scope.end = 120;

    auto alias_reader = placer::make_bam_reader(bam_path.string(), 1, alias_scope);
    REQUIRE(alias_reader != nullptr);
    REQUIRE(alias_reader->is_valid());
    REQUIRE(alias_reader->can_fetch());

    std::vector<std::string> alias_streamed;
    const int64_t alias_total = alias_reader->stream(
        [&](placer::BamRecordPtr&& record) {
            placer::ReadView view(record.get());
            alias_streamed.emplace_back(view.qname());
        });

    REQUIRE(alias_total == 1);
    REQUIRE(alias_streamed.size() == 1);
    REQUIRE(alias_streamed[0] == "chr1_in");

    std::vector<std::string> alias_fetched;
    const bool alias_fetch_ok = alias_reader->fetch(
        "1",
        0,
        1000,
        [&](placer::BamRecordPtr&& record) {
            placer::ReadView view(record.get());
            alias_fetched.emplace_back(view.qname());
            return true;
        });

    REQUIRE(alias_fetch_ok);
    REQUIRE(alias_fetched.size() == 1);
    REQUIRE(alias_fetched[0] == "chr1_in");
