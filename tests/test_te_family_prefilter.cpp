...
static void test_traceback_alignment_ignores_query_flanks() {
    using namespace placer;

    const std::string query = "GGGGGACGTACGTACGTTTTT";
    const std::string target = "ACGTACGTACGT";
    ...
    assert(exact.query_start == 5);
    assert(exact.query_end == 17);
    assert(exact.target_start == 0);
    assert(exact.target_end == 12);
    assert(exact.matches == 12);
    assert(exact.mismatches == 0);
    assert(exact.query_coverage == 12.0 / static_cast<double>(query.size()));
    assert(exact.query_interval_coverage == 1.0);
    assert(exact.target_coverage == 1.0);
    assert(exact.identity == 1.0);
}
...
int main() {
    ...
    test_traceback_alignment_ignores_query_flanks();
