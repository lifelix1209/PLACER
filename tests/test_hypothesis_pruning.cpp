...
int main() {
...

    {
        ...
        const auto unilateral_clip = make_summary(
            ...
            74,
            {"e1", "e2"});
        const auto supported_unilateral_clip = make_summary(
            5,
            340,
            340,
            0,
        ...
        assert(pipeline.should_keep_hypothesis_for_expensive_stage(supported_unilateral_clip, false));
        assert(pipeline.should_record_hypothesis_in_evidence_ledger(unilateral_clip));
    }

    {
        ...
        assert(selected[3].original_index == 5);
    }
...

    {
        Pipeline::HypothesisValidatorEvidence primary;
        primary.summary.original_index = 0;
        primary.summary.bp_left = 38194099;
        primary.summary.bp_right = 38194235;
        primary.summary.hypothesis_priority = 2;
        primary.summary.hypothesis_support = 4;
        primary.summary.hypothesis_score = 28;
        primary.summary.support_qnames = {"p1", "p2", "p3", "p4"};
        primary.precise_support = 7;
        primary.full_context_input_reads = 2;
        primary.left_anchor_input_reads = 4;
        primary.right_anchor_input_reads = 6;
        primary.feasible_for_expensive_stage = true;

        Pipeline::HypothesisValidatorEvidence strong_challenger = primary;
        strong_challenger.summary.original_index = 1;
        strong_challenger.summary.bp_left = 38193872;
        strong_challenger.summary.bp_right = 38193872;
        strong_challenger.summary.hypothesis_priority = 1;
        strong_challenger.summary.hypothesis_support = 5;
        strong_challenger.summary.hypothesis_score = 40;
        strong_challenger.summary.support_qnames = {"a1", "a2"};
        strong_challenger.precise_support = 5;
        strong_challenger.left_anchor_input_reads = 5;
        strong_challenger.right_anchor_input_reads = 5;

        Pipeline::HypothesisValidatorEvidence local_challenger = primary;
        local_challenger.summary.original_index = 2;
        local_challenger.summary.bp_left = 38194224;
        local_challenger.summary.bp_right = 38194235;
        local_challenger.summary.hypothesis_priority = 2;
        local_challenger.summary.hypothesis_support = 3;
        local_challenger.summary.hypothesis_score = 21;
        local_challenger.summary.support_qnames = {"b1", "b2"};
        local_challenger.precise_support = 5;
        local_challenger.left_anchor_input_reads = 3;
        local_challenger.right_anchor_input_reads = 5;

        Pipeline::HypothesisValidatorEvidence rescue;
        rescue.summary.original_index = 3;
        rescue.summary.bp_left = 38195163;
        rescue.summary.bp_right = 38195163;
        rescue.summary.hypothesis_priority = 0;
        rescue.summary.hypothesis_support = 1;
        rescue.summary.hypothesis_score = 8;
        rescue.summary.support_qnames = {"r1"};
        rescue.precise_support = 2;
        rescue.full_context_input_reads = 2;
        rescue.left_anchor_input_reads = 2;
        rescue.right_anchor_input_reads = 2;
        rescue.feasible_for_expensive_stage = true;

        const auto shortlist = pipeline.build_expensive_stage_shortlist(
            {primary, strong_challenger, local_challenger, rescue});
        assert(shortlist.size() == 4);
        assert(shortlist[3].validator.summary.original_index == 3);
    }

    {
        std::vector<Pipeline::HypothesisValidatorEvidence> independent_events;
        for (size_t i = 0; i < 5; ++i) {
            Pipeline::HypothesisValidatorEvidence event;
            event.summary.original_index = i;
            event.summary.bp_left = static_cast<int32_t>(1000 + (i * 700));
            event.summary.bp_right = event.summary.bp_left;
            event.summary.hypothesis_priority = 2;
            event.summary.hypothesis_support = 2;
            event.summary.hypothesis_score = 14.0;
            event.summary.support_qnames = {
                "event_" + std::to_string(i) + "_a",
                "event_" + std::to_string(i) + "_b",
            };
            event.event_evidence.bp_left = event.summary.bp_left;
            event.event_evidence.bp_right = event.summary.bp_right;
            event.event_evidence.support_qnames = event.summary.support_qnames;
            event.precise_support = 2;
            event.full_context_input_reads = 1;
            event.left_anchor_input_reads = 2;
            event.right_anchor_input_reads = 2;
            event.feasible_for_expensive_stage = true;
            independent_events.push_back(event);
        }

        const auto shortlist = pipeline.build_expensive_stage_shortlist(independent_events);
        assert(shortlist.size() == independent_events.size());
        for (size_t i = 0; i < shortlist.size(); ++i) {
            assert(shortlist[i].validator.summary.original_index == i);
        }
    }

    {
        Pipeline::BreakpointHypothesis strong_indel;
        strong_indel.valid = true;
        strong_indel.left = 38193872;
        strong_indel.right = 38193872;
        strong_indel.center = 38193872;
        strong_indel.support = 5;
        strong_indel.priority = 1;

        Pipeline::BreakpointHypothesis raw_split_pair;
        raw_split_pair.valid = true;
        raw_split_pair.left = 38194099;
        raw_split_pair.right = 38194235;
        raw_split_pair.center = 38194167;
        raw_split_pair.support = 4;
        raw_split_pair.priority = 2;

        Pipeline::BreakpointHypothesis local_raw_split;
        local_raw_split.valid = true;
        local_raw_split.left = 38194224;
        local_raw_split.right = 38194235;
        local_raw_split.center = 38194229;
        local_raw_split.support = 3;
        local_raw_split.priority = 2;

        Pipeline::BreakpointHypothesis truth_near_split;
        truth_near_split.valid = true;
        truth_near_split.left = 38195163;
        truth_near_split.right = 38195163;
        truth_near_split.center = 38195163;
        truth_near_split.support = 1;
        truth_near_split.priority = 0;

        const auto selected = pipeline.select_diverse_breakpoint_hypotheses(
            {strong_indel, raw_split_pair, local_raw_split, truth_near_split},
            3,
            38194216);

        assert(selected.size() == 4);
        assert(selected[3].center == 38195163);
    }
