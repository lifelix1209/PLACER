...
placer::InsertionFragment make_indel_fragment(
    const std::string& read_id,
    int32_t ref_junc_pos,
    int32_t length) {
    placer::InsertionFragment frag;
    frag.chrom = "chr9";
    frag.anchor_pos = 24897035;
    frag.read_id = read_id;
    frag.source = placer::InsertionFragmentSource::kCigarInsertion;
    frag.ref_side = placer::ReferenceSide::kUnknown;
    frag.ref_junc_pos = ref_junc_pos;
    frag.start = 100;
    frag.length = length;
    frag.read_len = 1200;
    frag.sequence = std::string(static_cast<size_t>(length), 'C');
    return frag;
}
...
int main() {
...

    {
        ...
        component.tid = 0;
        component.anchor_pos = 24897035;

        BreakpointCandidate seed_left;
        seed_left.pos = 24897034;
        component.breakpoint_candidates.push_back(seed_left);
        BreakpointCandidate seed_right;
        seed_right.pos = 24897249;
        component.breakpoint_candidates.push_back(seed_right);

        std::vector<const bam1_t*> local_records;
        std::vector<InsertionFragment> fragments;
        fragments.push_back(make_indel_fragment("truth_indel", 24897900, 397));
        ...
        assert(has_hypothesis(hypotheses, 24897900, 24897900));
    }

    {
        ...
        component.chrom = "chr9";
        component.tid = 0;
        component.anchor_pos = 33931292;

        BreakpointCandidate seed;
        seed.pos = 33931292;
        component.breakpoint_candidates.push_back(seed);

        std::vector<const bam1_t*> local_records;
        std::vector<InsertionFragment> fragments;
        fragments.push_back(make_indel_fragment("long_read_far_indel", 33954160, 960));
        ...
        assert(!has_hypothesis(hypotheses, 33954160, 33954160));
    }
