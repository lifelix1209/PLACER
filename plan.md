# PLACER TE Calling Improvement Plan (Post-v0.0.1, Low-Quality TE Reference Track)
...
Additional evidence for low-quality TE-library settings:

- Sniffles2 and related long-read SV methods show that repeat-aware clustering and adaptive filtering improve candidate stability before annotation.
- rMETL emphasizes realignment-based support for noisy long-read MEI signals.
- Winnowmap2 shows repeat-aware mapping can materially reduce downstream variant bias in repetitive regions.
- RepeatModeler2 and EDTA demonstrate that de novo repeat discovery/library refinement is necessary when reference TE libraries are incomplete or noisy.
- pantera shows pangenome-scale TE insertion polymorphism can be used to improve active-family representation.
...
### A7. Open-set TE classification (for poor TE reference quality)

- Split outputs into three states:
- `TE_CERTAIN`
- `TE_UNCERTAIN` (`UNK`)
- `NON_TE`
- Do not hard-drop insertion events only due to low family confidence.
- Keep posterior margin and confidence for downstream filtering.

### A8. Two-pass bootstrap TE library refinement

- Pass-1: run detection-first pipeline with open-set TE classification and collect high-confidence `UNK` consensus sequences.
- Cluster `UNK` consensus sequences and build an incremental sample-specific TE library.
- Pass-2: reclassify candidates using `base_library + incremental_library`.
- Keep conservative acceptance rules to avoid contaminating the library with non-TE artifacts.

### A9. Structural-evidence-dominant scoring under weak TE library

- Increase weight of split/SA consistency, breakpoint coherence, and assembly consistency.
- Keep TE k-mer identity as one feature, not a hard decision axis.
- Integrate hallmark evidence (polyA/TSD/orientation consistency) as independent likelihood terms.
...
- Done: Stage-A TE-name hard dependency removed.
- Done: assembly fragment selection decoupled from `te_name`.
- Done: post-assembly `TE=UNK` pass path (`PASS_INSERTION_TE_UNCERTAIN`).
- Done: stage-wise reject counters added to pipeline summary output.

Next implementation block:

1. Add explicit `te_status` in final output:
   - `TE_CERTAIN`, `TE_UNCERTAIN`, `NON_TE`.
2. Add posterior score fields:
   - `te_posterior_top1`, `te_posterior_top2`, `te_posterior_margin`.
...
### M4.5 (Open-set Classification Output)

- Introduce explicit `te_status` and posterior margin fields in outputs.
- Enforce no-drop policy for high-confidence insertion with uncertain TE class.

Exit criteria:

- `TE_UNCERTAIN` calls are measurable and auditable.
- Family-level precision is evaluated only on `TE_CERTAIN` subset.
...
### M7 (Bootstrap Library Refinement)

- Implement two-pass workflow with sample-specific incremental TE library.
- Run de novo repeat discovery/curation hooks for poor-library datasets.

Exit criteria:

- Recall increase relative to single-pass baseline.
- No large precision collapse on known negatives.
- `TE_UNCERTAIN` fraction decreases after pass-2.
...
- Open-set metrics:
- `TE_CERTAIN` precision
- `TE_UNCERTAIN` recall contribution
- Unknown-rate before/after pass-2 bootstrap

Required outputs:

- Stage-wise funnel table from `components` to `final_calls`.
- Separate accounting of `TE-certain` vs `TE-uncertain` insertion calls.
- Pass-1 vs pass-2 delta report (`final_calls`, recall, precision, unknown-rate).
...
- rMETL (Bioinformatics 2019): https://doi.org/10.1093/bioinformatics/btz106
- Winnowmap2 (Nat Methods 2022): https://doi.org/10.1038/s41592-022-01457-8
- RepeatModeler2 (PNAS 2020): https://doi.org/10.1073/pnas.1921046117
- EDTA (Genome Biol 2019): https://doi.org/10.1186/s13059-019-1905-y
- pantera (Nat Commun 2024): https://pubmed.ncbi.nlm.nih.gov/38926873/
