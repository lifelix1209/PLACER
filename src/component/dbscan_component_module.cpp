...
// dbscan_cluster_indices does an O(n^2) pairwise pass over each block. A
// single high-coverage/repetitive locus can otherwise accumulate thousands of
// signatures into one compatibility-window block, making that one block cost
// dominate (and, under the bounded/ordered parallel reducer, stall the whole
// run). Cap block size so worst-case per-block cost stays bounded regardless
// of local coverage/repeat density.
constexpr size_t kMaxBlockSignatures = 1500;
...
bool append_insertion_signatures_from_cigar(
    const ReadView& read,
    size_t read_index,
...
std::vector<InsertionSignature> extract_signatures(
    ...
    for (size_t read_idx = 0; read_idx < records.size(); ++read_idx) {
        ...
        if (!emitted_cigar && !emitted_split) {
...
std::vector<SignatureBlock> build_blocks(const std::vector<InsertionSignature>& signs) {
    ...
    // Split any oversized block into contiguous, position-sorted chunks so
    // the O(n^2) DBSCAN pass below never runs against more than
    // kMaxBlockSignatures signatures at once.
    std::vector<SignatureBlock> capped_blocks;
    capped_blocks.reserve(blocks.size());
    for (const SignatureBlock& block : blocks) {
        size_t chunk_begin = block.begin;
        while (block.end - chunk_begin > kMaxBlockSignatures) {
            capped_blocks.push_back(SignatureBlock{chunk_begin, chunk_begin + kMaxBlockSignatures});
            chunk_begin += kMaxBlockSignatures;
        }
        capped_blocks.push_back(SignatureBlock{chunk_begin, block.end});
    }
