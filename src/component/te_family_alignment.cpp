...
AffineAlignmentCell extend_diag(
    ...
    next.query_bases += 1;
    next.target_bases += 1;
    if (next.query_start < 0) {
        next.query_start = query_pos;
    }
    if (next.target_start < 0) {
        next.target_start = target_pos;
    }
    next.query_end = query_pos + 1;
    next.target_end = target_pos + 1;
    if (match) {
        next.matches += 1;
    } else {
        next.mismatches += 1;
...
AffineAlignmentCell start_local_diag(bool match, int32_t query_pos, int32_t target_pos) {
    if (!match) {
        return AffineAlignmentCell{};
    }
    ...
    cell.query_bases = 1;
    cell.target_bases = 1;
    cell.query_start = query_pos;
    cell.query_end = query_pos + 1;
    cell.target_start = target_pos;
    cell.target_end = target_pos + 1;
    cell.valid = true;
    return cell;
}
...
ExactAlignmentSummary banded_semiglobal_affine_align(
    ...
    for (int32_t i = 1; i <= n; ++i) {
        ...
        for (int32_t j = j_min; j <= j_max; ++j) {
            ...
            AffineAlignmentCell best_match = start_local_diag(match, i - 1, j - 1);
            ...
            AffineAlignmentCell best_ins = reset_if_nonpositive(
                extend_up(match_prev[idx], kGapOpenPenalty, i - 1));
            const AffineAlignmentCell ins_from_ins = reset_if_nonpositive(
                extend_up(ins_prev[idx], kGapExtendPenalty, i - 1));
            ...
            const AffineAlignmentCell ins_from_del = reset_if_nonpositive(
                extend_up(del_prev[idx], kGapOpenPenalty, i - 1));
            ...
            AffineAlignmentCell best_del = reset_if_nonpositive(
                extend_left(match_curr[prev_idx], kGapOpenPenalty, j - 1));
            const AffineAlignmentCell del_from_del = reset_if_nonpositive(
                extend_left(del_curr[prev_idx], kGapExtendPenalty, j - 1));
            ...
            const AffineAlignmentCell del_from_ins = reset_if_nonpositive(
                extend_left(ins_curr[prev_idx], kGapOpenPenalty, j - 1));
    ...
    summary.query_interval_coverage =
        best.query_end > best.query_start
            ? static_cast<double>(best.query_bases) /
                  static_cast<double>(best.query_end - best.query_start)
            : 0.0;
    summary.target_coverage = static_cast<double>(best.target_bases) /
        static_cast<double>(std::max(1, m));
    summary.score = summary.identity * summary.query_coverage;
    summary.query_start = best.query_start;
    summary.query_end = best.query_end;
    summary.target_start = best.target_start;
    summary.target_end = best.target_end;
    summary.matches = best.matches;
    summary.mismatches = best.mismatches;
    summary.insertions = best.insertions;
    summary.deletions = best.deletions;
    summary.aligned_query_bases = best.query_bases;
    summary.aligned_target_bases = best.target_bases;
    summary.alignment_columns = best.alignment_len;
    return summary;
}
