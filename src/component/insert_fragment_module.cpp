...
constexpr int32_t kComponentLocalInsertionRetentionBp = 1000;
constexpr int32_t kSplitSaInsertionMinLen = 50;
constexpr size_t kPrimaryInsertionFragmentsPerRead = 2;
constexpr size_t kMaxInsertionFragmentsPerRead = 4;
...
int32_t split_sa_insertion_len(
    const NormalizedAln& a,
    const NormalizedAln& b) {
    if (a.chrom != b.chrom || a.is_reverse != b.is_reverse) {
        return -1;
    }

    const NormalizedAln* left = &a;
    const NormalizedAln* right = &b;
    if (b.qstart < a.qstart) {
        left = &b;
        right = &a;
    }
...
bool has_split_sa_query_gap_insertion(
    const NormalizedAln& a,
    const NormalizedAln& b) {
    return split_sa_insertion_len(a, b) >= kSplitSaInsertionMinLen;
}
...
bool same_insertion_op(const InsOp& a, const InsOp& b) {
    return a.start == b.start && a.len == b.len && a.ref_pos == b.ref_pos;
}

bool insertion_is_component_local(const InsOp& op, const ComponentCall& component) {
    if (op.ref_pos < 0) {
        return false;
    }
    if (component.anchor_pos >= 0 &&
        std::abs(op.ref_pos - component.anchor_pos) <= kComponentLocalInsertionRetentionBp) {
        return true;
    }
    for (const auto& candidate : component.breakpoint_candidates) {
        if (candidate.pos >= 0 &&
            std::abs(op.ref_pos - candidate.pos) <= kComponentLocalInsertionRetentionBp) {
            return true;
        }
    }
    return false;
...
std::vector<InsOp> select_insertions_for_fragment_extraction(
    std::vector<InsOp> ins_ops,
    ...
    std::vector<InsOp> selected;
    selected.reserve(std::min(ins_ops.size(), kMaxInsertionFragmentsPerRead));

    auto append_unique = [&](const InsOp& op) {
        if (selected.size() >= kMaxInsertionFragmentsPerRead) {
            return;
        }
        const bool exists = std::any_of(
            ...
            selected.end(),
            [&](const InsOp& kept) {
                return same_insertion_op(kept, op);
            });
        if (!exists) {
    ...
    for (const auto& op : ins_ops) {
        if (selected.size() >= kPrimaryInsertionFragmentsPerRead) {
            break;
        }
        append_unique(op);
    }
    ...
    for (size_t idx_in_bin : component.read_indices) {
        ...
        if (flank_idx >= 0) {
            ...
            if (mate_idx >= 0 && best_nonoverlap >= min_len) {
                ...
                if (!has_split_sa_query_gap_insertion(flank, mate)) {
                    continue;
                }
        ...
        for (const auto& row : sa_all) {
            ...
            if (!has_split_sa_query_gap_insertion(primary, sa)) {
                continue;
            }
...
std::vector<InsertionFragment> CigarInsertionFragmentModule::extract(
    ...
    for (size_t idx_in_bin : indices) {
        ...
        auto ins_ops = (read.mapq() == kInsertionCandidateRequiredMapq)
            ? find_long_insertions(read, min_ins)
            : std::vector<InsOp>{};
        if (!ins_ops.empty()) {
        ...
        if (!ins_ops.empty()) {
            ins_ops = select_insertions_for_fragment_extraction(std::move(ins_ops), component);
