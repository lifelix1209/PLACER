...
int32_t resolve_header_tid(bam_hdr_t* header, const std::string& chrom) {
    if (header == nullptr) {
        return -1;
    }
    for (const std::string& alias : contig_name_aliases(chrom)) {
        const int32_t tid = sam_hdr_name2tid(header, alias.c_str());
        if (tid >= 0) {
            return tid;
        }
    }
...
public:
    ...
    bool fetch(
        ...
        const int32_t tid = resolve_header_tid(fetch_header_, chrom);
        if (tid < 0) {
            return false;
        }
        ...
        if (region_scope_.enabled) {
            const int32_t scope_tid = resolve_header_tid(fetch_header_, region_scope_.chrom);
            if (scope_tid < 0 || tid != scope_tid) {
                return false;
            }
...
private:
    ...
    int64_t stream_scoped(
        ...
        const int32_t tid = resolve_header_tid(header_, region_scope_.chrom);
        if (tid < 0) {
